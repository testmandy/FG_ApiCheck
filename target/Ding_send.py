...
# jenkins登录地址
jenkins_url = "http://jenkins.otter-innov.net"
# 获取jenkins对象
server = jenkins.Jenkins(jenkins_url, username='test_user', password='test_2022@') #Jenkins登录名 ，密码
# job名称
job_name = "/job/ApiCheck_v1.5/" #Jenkins运行任务名称
# job的url地址
...
report_url2 = str(report_url).replace('121.41.41.232:9000', 'jenkins.otter-innov.net')
...
def DingTalkSend(token):
    ...
    con = {"msgtype": "text",
           "text": {
               "content": "Pytest_sass后台管理接口自动化定时检测执行完成。"
                          "\n测试概述:"
                          "\n运行总数:" + retries_run +
                          "\n通过数量:" + status_passed +
                          "\n失败数量:" + status_failed +
